...
//			$points = round($points,2);
                                //return individual assignment result
...
//                                $ppercent= _makeLetterGrade($total,"",User('STAFF_ID'),"%");
                                ...
                                if($points_RET[$THIS_RET['STUDENT_ID']][1]['PARTIAL_POINTS']!='')
                                    return ($total>$max_allowed?'<FONT color=red>':'').$ppercent.($total>$max_allowed?'</FONT>':'').'%&nbsp;<B>'._makeLetterGrade($total_m,"",User('STAFF_ID')).'</B>';
                                else
					return 'Not Graded!';
			}
			else
		...
		case 'POINTS':
			...
			elseif(strpos($value,'.'))
				$value = rtrim(rtrim($value,'0'),'.');
                        //return student itemized assignment results
